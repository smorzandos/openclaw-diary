...
部分安全扫描工具（如 VirusTotal）可能对本 skill 产生误报，常见警告及说明如下：

| 警告内容 | 实际情况 |
|---|---|
| 加密货币密钥相关代码 | 误报。文件中的 `App Secret`、`API Token` 等字段是飞书/Notion/Flomo 的普通 API 凭证，与加密货币无关 |
| 外部 API 调用 | 属实，但正常。Skill 会调用飞书等平台 API，所有凭证由用户自己提供，我们无法访问 |
| eval 等风险函数 | 误报。Skill 文件为 Markdown 格式，代码块均为说明文档，不是可执行代码 |

**为什么会触发误报？**

Skill 文件是 Markdown 格式，包含 API 字段名、外部平台链接和代码示例。部分扫描工具的启发式规则对这类内容敏感，容易对非可执行文件产生误判。

如果你对安全性有疑虑，欢迎直接审查源代码：所有 skill 文件均为纯文本，可在 GitHub 上公开查阅。
